...
| `bolt_alarm`   | 锁舌未到位报警【v5.1 新增, v5.3 废弃】 | 无             | 0x06 (status=2) |

> **v5.1 新增说明：** `door_closed`、`lock_success`、`bolt_alarm` 对应 STM32 的 `EVT_LOCK_STATUS (0x06)` 事件，通过 param 区分状态。
>
> **v5.3 废弃说明：** STM32 v2.8+ 已移除霍尔传感器，不再发送 `bolt_alarm` 事件。ESP32 保留处理代码以兼容旧版 STM32 (v2.7-)。
...
### 3.4 本地预览模式 (Local Preview Mode)【v5.3 新增】

> **说明：** 本地预览模式在 ESP32 的 LCD 屏幕上实时显示摄像头画面，视频数据不发送到服务器，仅在本地显示。

#### 3.4.1 启动本地预览 (Server → Device)
...
| error  | string | 错误描述（仅 status=error 时有效） |
...
- ESP32 端会自动检查互斥条件，如果冲突会返回错误响应
- 服务器端可选择性实现互斥检查，提前告知用户冲突
...
### 3.6 锁控命令 (Lock Control)

#### 3.6.1 远程开锁/关锁 (Server → Device)
...
### 3.7 硬件外设控制 (Dev Control)

#### 3.7.1 蜂鸣器控制 (Server → Device)
...
### 3.8 查询命令 (Query)【v5.2 新增】

> **v5.2 新增：** 支持服务器远程查询传感器数据和设备状态。

#### 3.8.1 查询传感器数据 (Server → Device)
...
### 3.10 心跳机制 (Heartbeat) - 预留功能

> **状态：** 已实现，暂不启用。后续可通过配置开启。

#### 3.10.1 心跳请求 (Device → Server)
...
| 版本 | 日期       | 变更说明                                                                                                                                                    |
| ---- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v5.3 | 2026-05-04 | 新增本地预览模式（local_preview）；废弃 bolt_alarm 事件（STM32 v2.8+ 移除霍尔传感器）；ESP32 保留处理代码以兼容旧版 STM32                                   |
