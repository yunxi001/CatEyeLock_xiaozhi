...
**method 取值与 STM32 对应关系：**

| method | 说明 | STM32 D0 值 | uid 说明 |
|--------|------|-------------|----------|
| `finger` | 指纹开锁 | 0x01 | 指纹 ID |
| `nfc` | NFC 开锁 | 0x02 | NFC 卡 ID |
| `pwd` | 密码开锁 | 0x03 | 固定为 0（无法识别用户） |
| `remote` | 远程开锁(人脸) | 0x04 | 人脸用户 ID |
| `key` | 机械钥匙 | 0x05 | 固定为 0（无法识别用户） |
...
| 功能 | 说明 |
|------|------|
| 定时状态上报 | 协议建议 60 秒定时上报，当前仅在状态变化时上报 |
| 心跳定时发送 | 已实现发送方法，但未启用定时器 |
| 服务器端 | 需配合开发服务器端协议处理 |
...
| 版本 | 日期 | 变更说明 |
|------|------|----------|
| v5.0 | 2025-12-10 | 整合协议，与 STM32 协议对齐，统一蜂鸣器/警报控制 |
| v4.0 | 2025-12-09 | 新增 msg_id/ACK 机制、用户管理闭环 |
| v3.0 | 2025-12-08 | 新增监控模式、人脸识别 |
| v2.0 | 2025-12-07 | 新增锁控命令、事件上报 |
| v1.0 | 2025-12-06 | 初始版本，基础音频通信 |
