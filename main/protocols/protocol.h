...
public:
     ...
     * @param msg    响应消息
     */
    virtual void SendAck(const std::string& msg_id, int code = 0, const std::string& msg = "OK");
     ...
     */
    virtual void SendStatusReport(int battery, int lux, int lock_state, int light_state);
    ...
    /**
     * @brief 发送开锁日志上报
     * @param method     开锁方式（fingerprint/nfc/password/remote/key）
     * @param uid        用户 ID
     * @param result     是否成功
     * @param fail_count 失败次数
     */
    virtual void SendLogReport(const std::string& method, int uid, bool result, int fail_count = 0);
     ...
     * @param msg      消息说明
     */
    virtual void SendUserMgmtResult(const std::string& category, const std::string& command,
