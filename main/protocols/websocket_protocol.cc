...
bool WebsocketProtocol::SendVideo(const uint8_t* data, size_t size, uint32_t timestamp, 
    ...
    ESP_LOGD(TAG, "发送视频帧: %dx%d, 大小=%zu, 时间戳=%u", 
 ...
 */
bool WebsocketProtocol::SendFaceRecognition(const uint8_t* jpeg_data, size_t jpeg_size,
                                             uint16_t width, uint16_t height) {
    if (websocket_ == nullptr || !websocket_->IsConnected()) {
...
bool WebsocketProtocol::OpenAudioChannel() {
    ...
    if (!websocket_->Connect(url.c_str())) {
        ESP_LOGE(TAG, "连接 WebSocket 服务器失败, 错误码=%d", websocket_->GetLastError());
...
void WebsocketProtocol::SendAck(const std::string& msg_id, int code, const std::string& msg) {
    cJSON* root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "type", "ack");
    cJSON_AddStringToObject(root, "msg_id", msg_id.c_str());
    ...
    ESP_LOGI(TAG, "发送 ACK: msg_id=%s, code=%d", msg_id.c_str(), code);
    SendText(message);
}

void WebsocketProtocol::SendStatusReport(int battery, int lux, int lock_state, int light_state) {
    cJSON* root = cJSON_CreateObject();
    ...
    ESP_LOGI(TAG, "发送状态上报: bat=%d%%, lux=%d, lock=%d, light=%d", 
             battery, lux, lock_state, light_state);
    SendText(message);
}
...
void WebsocketProtocol::SendLogReport(const std::string& method, int uid, bool result, int fail_count) {
    cJSON* root = cJSON_CreateObject();
...
void WebsocketProtocol::SendUserMgmtResult(const std::string& category, const std::string& command,
