...
#include <esp_log.h> // 应该在LOCAL_LOG_LEVEL定义之后包含
...
// 弱符号声明esp_video_deinit函数，如果未实现则返回ESP_ERR_NOT_SUPPORTED
...
std::string Esp32Camera::Explain(const std::string &question) {
  ...
  // 我们使用优化编码器（耗时约500毫秒和8KB
  // SRAM）来编码图像为JPEG，并为此创建一个线程。
...
bool Esp32Camera::CaptureJpeg(uint8_t **jpeg_data, size_t *jpeg_size,
  ...
  *jpeg_data = (uint8_t *)heap_caps_malloc(*jpeg_size,
                                           MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (*jpeg_data == nullptr) {
    ESP_LOGE(TAG, "Failed to allocate %u bytes for JPEG data",
             (unsigned)*jpeg_size);
    return false;
  }

  memcpy(*jpeg_data, jpeg_buffer.data(), *jpeg_size);
  // ESP_LOGI(TAG, "Captured JPEG: %dx%d, size=%zu bytes, quality=%d", w, h,
  // *jpeg_size, quality);
  return true;
}
