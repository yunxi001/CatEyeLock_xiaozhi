...
public:
  ...
  /**
   * @brief 捕获一帧RGB565原始数据用于本地预览
   * @details 与 CaptureForStream() 类似，但专门用于本地预览：
   *          - 只捕获一帧（不丢弃前两帧）
   *          - 不在屏幕上显示预览
   *          - 输出RGB565格式的原始数据
   *          - 专为本地预览优化
  ...
  /**
   * @brief 获取当前帧的RGB565数据指针
   * @details 必须在 CaptureForPreview() 成功后调用
   *          数据有效期直到下一次 Capture*() 调用
   * @return const uint8_t* RGB565数据指针，如果无数据返回nullptr
   */
  const uint8_t *GetRgb565Data() const;

  /**
   * @brief 获取当前帧的RGB565数据大小
   * @return size_t 数据大小（字节），如果无数据返回0
   */
