...
struct LockMessage {
  ...
  /** 判断是否为系统消息 */
  bool IsSys() const { return category == static_cast<uint8_t>(MsgCategory::SYS); }
  
  /** 判断是否为上报消息 */
  bool IsRpt() const { return category == static_cast<uint8_t>(MsgCategory::RPT); }
  
  /** 判断是否为控制命令 */
  bool IsCmd() const { return category == static_cast<uint8_t>(MsgCategory::CMD); }
  
  /** 判断是否为用户管理消息 */
  bool IsUser() const { return category == static_cast<uint8_t>(MsgCategory::USER); }
  
  /** 判断是否为成功应答 */
  bool IsAckOk() const { return IsSys() && type == static_cast<uint8_t>(SysType::ACK_OK); }
  
  /** 判断是否为错误应答 */
  bool IsAckErr() const { return IsSys() && type == static_cast<uint8_t>(SysType::ACK_ERR); }
};
