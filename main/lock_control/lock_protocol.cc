 ...
 * @brief 构建成功应答消息
 * 
 * 格式：CAT=0x00, TYPE=0x01, D0=原TYPE
 */
std::vector<uint8_t> LockProtocol::BuildAckOk(uint8_t orig_type) {
  std::array<uint8_t, 3> data = {orig_type, 0x00, 0x00};
 ...
 * @brief 构建错误应答消息
 * 
 * 格式：CAT=0x00, TYPE=0x00, D0=原TYPE, D1=错误码
 */
std::vector<uint8_t> LockProtocol::BuildAckErr(uint8_t orig_type, AckError error) {
  std::array<uint8_t, 3> data = {orig_type, static_cast<uint8_t>(error), 0x00};
  return BuildMessage(static_cast<uint8_t>(MsgCategory::SYS),
