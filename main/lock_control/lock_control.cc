...
LockControlService::LockControlService()
    : uart_port_(UART_NUM_0), running_(false), rx_task_handle_(nullptr),
      event_callback_(nullptr) {}

LockControlService::~LockControlService() { Stop(); }
...
bool LockControlService::Start(uart_port_t port, int tx_pin, int rx_pin) {
  ...
  // 设置 UART 引脚
  err = uart_set_pin(uart_port_, tx_pin, rx_pin, UART_PIN_NO_CHANGE,
                     UART_PIN_NO_CHANGE);
  if (err != ESP_OK) {
  ...
  ESP_LOGI(TAG, "UART 初始化完成，端口 %d (TX=%d, RX=%d)", uart_port_, tx_pin,
           rx_pin);
  return true;
}
...
void LockControlService::RxLoop() {
  ...
  while (running_) {
    ...
    if (pos >= LOCK_PROTOCOL_LENGTH) {
      LockMessage msg =
          LockProtocol::ParseMessage(buffer, LOCK_PROTOCOL_LENGTH);

      if (msg.valid) {
        ESP_LOGI(TAG,
                 "收到消息: CAT=0x%02X, TYPE=0x%02X, D=[0x%02X,0x%02X,0x%02X]",
...
bool LockControlService::SendMessage(uint8_t cat, uint8_t type,
  ...
  for (int retry = 0; retry < MAX_RETRIES; retry++) {
    ...
    if (len == static_cast<int>(msg.size())) {
      ESP_LOGD(TAG,
               "发送消息: CAT=0x%02X, TYPE=0x%02X, D=[0x%02X,0x%02X,0x%02X]",
...
bool LockControlService::SendLock(LockMode mode, uint8_t hold_seconds) {
  return SendMessage(
      static_cast<uint8_t>(MsgCategory::CMD),
      static_cast<uint8_t>(CmdType::CMD_LOCK),
      {static_cast<uint8_t>(mode), hold_seconds, LOCK_PROTOCOL_EMPTY});
}
...
bool LockControlService::SendLockDoor() { return SendLock(LockMode::LOCK, 0); }

bool LockControlService::SendOledIcon(OledIcon icon) {
  return SendMessage(
      static_cast<uint8_t>(MsgCategory::CMD),
      static_cast<uint8_t>(CmdType::CMD_OLED),
      {static_cast<uint8_t>(icon), LOCK_PROTOCOL_EMPTY, LOCK_PROTOCOL_EMPTY});
}
...
bool LockControlService::SendBeep(uint8_t count, BeepFreq freq) {
  return SendMessage(static_cast<uint8_t>(MsgCategory::CMD),
                     ...
                     {count, static_cast<uint8_t>(freq), LOCK_PROTOCOL_EMPTY});
}

bool LockControlService::SendSyncTime(uint8_t hour, uint8_t minute,
                                      uint8_t second) {
  return SendMessage(static_cast<uint8_t>(MsgCategory::CMD),
...
bool LockControlService::SendLight(LightMode mode) {
  return SendMessage(
      ...
      {static_cast<uint8_t>(mode), LOCK_PROTOCOL_EMPTY, LOCK_PROTOCOL_EMPTY});
}
...
bool LockControlService::QuerySensors() {
  return SendMessage(
      ...
      {LOCK_PROTOCOL_EMPTY, LOCK_PROTOCOL_EMPTY, LOCK_PROTOCOL_EMPTY});
}
...
bool LockControlService::FingerprintEnroll(uint8_t expected_id) {
  return SendMessage(static_cast<uint8_t>(MsgCategory::USER),
                     ...
                     {static_cast<uint8_t>(FpSubCmd::FP_ENROLL), expected_id,
                      LOCK_PROTOCOL_EMPTY});
}
...
bool LockControlService::SetPassword(uint32_t password) {
  ...
  return SendMessage(static_cast<uint8_t>(MsgCategory::USER),
                     static_cast<uint8_t>(UserPwdCmd::PWD_SET), encoded);
}
...
bool LockControlService::SetTempPassword(uint32_t password, uint32_t expires) {
  ...
  // 第1包：发送密码
  std::array<uint8_t, 3> pwd_encoded =
      LockProtocol::EncodePasswordHex(password);
  bool ok =
      SendMessage(static_cast<uint8_t>(MsgCategory::USER),
                  static_cast<uint8_t>(UserPwdCmd::TEMP_PWD_SET), pwd_encoded);
  if (!ok) {
  ...
  ok = SendMessage(static_cast<uint8_t>(MsgCategory::USER),
                   static_cast<uint8_t>(UserPwdCmd::TEMP_PWD_EXP), exp_encoded);
  if (!ok) {
...
bool LockControlService::SendPing() {
  return SendMessage(
      static_cast<uint8_t>(MsgCategory::SYS),
