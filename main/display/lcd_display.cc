...
bool LcdDisplay::EnterPreviewMode() {
  DisplayLockGuard lock(this);

  if (preview_mode_active_) {
    ESP_LOGW(TAG, "已处于预览模式");
    return true;
  }
  ...
  // 3. 分配 PSRAM 缓冲区
  preview_canvas_buffer_ = heap_caps_malloc(buffer_size, MALLOC_CAP_SPIRAM);
  if (preview_canvas_buffer_ == nullptr) {
    ESP_LOGE(TAG, "无法分配 PSRAM 缓冲区：%zu 字节", buffer_size);
    // 恢复 UI 组件显示
    ExitPreviewMode();
    return false;
  }
  ...
  // 初始化缓冲区为黑色
  memset(preview_canvas_buffer_, 0, buffer_size);

  ESP_LOGI(TAG, "已分配 PSRAM 缓冲区：%zu 字节", buffer_size);

  // 4. 创建全屏 Canvas 对象
  auto screen = lv_screen_active();
  preview_canvas_ = lv_canvas_create(screen);
  if (preview_canvas_ == nullptr) {
    ESP_LOGE(TAG, "无法创建 Canvas 对象");
    heap_caps_free(preview_canvas_buffer_);
    preview_canvas_buffer_ = nullptr;
    ExitPreviewMode();
    return false;
  }

  // 5. 配置 Canvas 为 RGB565 格式
  lv_canvas_set_buffer(preview_canvas_, preview_canvas_buffer_, width_, height_,
                       LV_COLOR_FORMAT_RGB565);

  // 6. 设置 Canvas 为全屏
  lv_obj_set_size(preview_canvas_, width_, height_);
  lv_obj_align(preview_canvas_, LV_ALIGN_CENTER, 0, 0);

  // 7. 设置预览模式标志
  preview_mode_active_ = true;

  // 8. 创建"预览中"指示器（可选）
  preview_indicator_ = lv_label_create(screen);
  if (preview_indicator_ != nullptr) {
    // 设置文本
    lv_label_set_text(preview_indicator_, "预览中");

    // 设置样式：白色文字，半透明黑色背景
    lv_obj_set_style_bg_color(preview_indicator_, lv_color_hex(0x000000),
                              LV_PART_MAIN);
    lv_obj_set_style_bg_opa(preview_indicator_, LV_OPA_70, LV_PART_MAIN);
    lv_obj_set_style_text_color(preview_indicator_, lv_color_hex(0xFFFFFF),
                                LV_PART_MAIN);
    lv_obj_set_style_pad_all(preview_indicator_, 4, LV_PART_MAIN);
    lv_obj_set_style_radius(preview_indicator_, 4, LV_PART_MAIN);

    // 定位到右上角
    lv_obj_align(preview_indicator_, LV_ALIGN_TOP_RIGHT, -5, 5);

    ESP_LOGI(TAG, "已创建预览指示器");
  }

  ESP_LOGI(TAG, "预览模式已激活（%dx%d）", width_, height_);
  return true;
}

void LcdDisplay::ExitPreviewMode() {
  DisplayLockGuard lock(this);

  if (!preview_mode_active_) {
    ESP_LOGD(TAG, "未处于预览模式，无需退出");
    return;
  }
  ...
  // 1. 销毁"预览中"指示器
  if (preview_indicator_ != nullptr) {
    lv_obj_del(preview_indicator_);
    preview_indicator_ = nullptr;
    ESP_LOGI(TAG, "已销毁预览指示器");
  }

  // 2. 销毁 Canvas 对象
  if (preview_canvas_ != nullptr) {
    lv_obj_del(preview_canvas_);
  ...
  // 5. 设置预览模式标志为 false
  preview_mode_active_ = false;

  ESP_LOGI(TAG, "预览模式已退出");
}

bool LcdDisplay::UpdatePreviewCanvas(const uint8_t *rgb565_data, uint16_t width,
                                     uint16_t height) {
  DisplayLockGuard lock(this);

  // 1. 检查是否处于预览模式
  if (!preview_mode_active_) {
    ESP_LOGW(TAG, "未处于预览模式，无法更新 Canvas");
    return false;
  }

  // 2. 检查 Canvas 对象和缓冲区是否有效
  if (preview_canvas_ == nullptr || preview_canvas_buffer_ == nullptr) {
    ESP_LOGE(TAG, "Canvas 对象或缓冲区无效 (canvas=%p, buffer=%p)",
             preview_canvas_, preview_canvas_buffer_);
    return false;
  }

  // 3. 检查数据指针是否有效
  if (rgb565_data == nullptr) {
    ESP_LOGE(TAG, "RGB565 数据指针为空");
    return false;
  }
