...
void McpServer::AddCommonTools() {
  ...
  if (camera) {
    ...
    if (lcd_display) {
      AddTool(
          "self.camera.show_peephole",
          "Show or hide the peephole camera view on the device screen. "
          "Use this tool when the user wants to see who is at the door, check "
          "the camera view, or view the monitoring screen.\n"
          "Args:\n"
          "  `enable`: true to show the camera view, false to hide it.",
          ...
          [](const PropertyList &properties) -> ReturnValue {
            ...
            if (enable) {
              ...
              if (app.IsLocalPreviewActive()) {
                return "Peephole view is already showing";
              }

              if (app.IsMonitorMode()) {
                throw std::runtime_error("Cannot show peephole view while "
                                         "remote monitoring is active");
              }

              if (!app.StartLocalPreview()) {
                throw std::runtime_error(
                    "Failed to show peephole view. The camera may be busy.");
              }

              return "Peephole view is now showing on the screen. You can see "
                     "who is at the door.";
            } else {
              // 关闭猫眼画面显示
              if (!app.IsLocalPreviewActive()) {
                return "Peephole view is not showing";
              }

              app.StopLocalPreview();
              return "Peephole view closed";
            }
          });
    }
  }
