...
bool Assets::InitializePartition() {
  ...
  ESP_LOGI(TAG, "The storage free size is %u KB",
           (unsigned)(storage_size / 1024));
  ESP_LOGI(TAG, "The partition size is %u KB",
           (unsigned)(partition_->size / 1024));
  if (storage_size < partition_->size) {
    ESP_LOGE(
        TAG, "The free size %u KB is less than assets partition required %u KB",
        (unsigned)(storage_size / 1024), (unsigned)(partition_->size / 1024));
    return false;
  }
...
bool Assets::Download(
  ...
  size_t sectors_to_erase =
      (content_length + SECTOR_SIZE - 1) / SECTOR_SIZE; // 向上取整
